...
func (s *NetworkGetSuite) createCommand(c *gc.C) cmd.Command {
	...
	// This should not happen. A hostname should never population the
	// address field. However, until the code is updated to prevent
	// addresses from being populated by Hostnames (e.g. Change the Address
	// field type from `string` to `net.IP` which will ensure all code paths
	// exclude string population) we will have this check in place to ensure
	// that hostnames are resolved to IPs and that hostnames populate a
	// distinct field. `network-get --primary-address` and the like should
	// only ever return IPs.
